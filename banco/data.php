<?php
	...
	switch ($comandos['acao']) {
		...
		case "validar_login":
			login($comandos['usuario'], $comandos['senha']);
		break;
	...
	/*
	if($comandos['acao'] === 'buscar-dados-substancia') {
		buscar($comandos['substancia']);
	} elseif($comandos['acao'] === 'informacoes_cadastradas') {
		buscarInfoVidrarias();
	} elseif($comandos['acao'] === 'validar_login') {
		login($comandos['usuario'], $comandos['senha']);
	} elseif($comandos['acao'] === 'cadastrar_usuario') {
		cadastrarUsuario($comandos['nome'], $comandos['email'], $comandos['usuario'], $comandos['senha'], $comandos['acesso']);
	} elseif($comandos['acao'] === 'carregar_id_pratica') {
		carregarIDPratica2();
	}
	*/
