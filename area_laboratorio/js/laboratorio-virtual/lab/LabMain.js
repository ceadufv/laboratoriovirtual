...
function construir_tab(tab, s) {
    ...
    if (s.disponivel == 'N') {
        ...
        botao = '<button data-type="' + s.conceito + '" data-id="' + s.id + '" type="button" class="btn btn-dark m-3 botao" onClick="Armario.selecionarItem(this);">Selecionar</button>';
        class_opcao = '';
    }
...
function getItensSelecionadosArmario() {
    return 'getItensSelecionadosArmario';
    var result = [];
    $('#armario button[data-marcado="true"]').each(function () {
        result.push({
