...
class DropZone{
    static add(){
        ...
        for(var i=0;i<zones.length;i++){
            ...
            var zone = GAME_SCENE.add.zone(zones[i].x, zones[i].y).setCircleDropZone(40);
    ...
    */
    ...
    insertDrag(obj1){
        ...
        obj1.on('dragenter', function (pointer, dropZone) {
            //dropZone.teste.setTint(0x00ff00);
            console.log('dragenter');
        });

        obj1.on('dragleave', function (pointer, dropZone) {
            console.log('dragleave');
            //dropZone.teste.clearTint();
        });
